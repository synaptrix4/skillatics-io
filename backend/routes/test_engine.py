...
def start_test():
	...
	type_filter = data.get("type") if data and data.get("type") in ["Aptitude", "Technical"] else None
...
def submit_answer():
	...
	mongo.db.test_sessions.update_one(
		...
		{"$set": {"currentDifficulty": new_diff, "adaptivePath": adaptive_path, "askedQuestionIds": asked},
